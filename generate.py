...
from model.video_music__mixer_mamba import VideoMusicMixerMamba
from model.video_regression import VideoRegression

from dataset.vevo_dataset import create_vevo_datasets


version = "2025_1_VMM_contrastive_11_MuVi-Sync_best_predictMuVi-Sync"
split_ver = SPLIT_VER
...
# custumPrimer = ["N","N","Em","Em","Em","G","CM7","CM7","F","F","F","Bm7","Bm7","Bm7","Am","Am"]
# custumPrimer = ["N","N","N","N","N","N","N","Am","Am","E","Am","Am","E","Am","Am","Am","Am"]
...
def main(test_id):
    testFileList=[]
    valFileList=[]
    # os.path.exists("dataset/dataset/vevo_meta/split/v1/test.txt")
    args = parse_generate_args()
    if args.is_pop909:
        with open("dataset/dataset_pop909/pop909_meta/split/test.txt") as txt_file:
            for line in txt_file:
                testFileList.append(line.strip())
        with open('dataset/dataset_pop909/pop909_meta/split/val.txt') as txt_file:
            for line in txt_file:
                valFileList.append(line.strip())
    else:
        with open("dataset/dataset/vevo_meta/split/"+ split_ver +'/test.txt') as txt_file:
            for line in txt_file:
                testFileList.append(line.strip())
        with open('dataset/dataset/vevo_meta/split/'+ split_ver +'/val.txt') as txt_file:
            for line in txt_file:
                valFileList.append(line.strip())

    with open('dataset/dataset/vevo_meta/chord.json') as json_file:
    ...
    print(f"model_weights_path:{args.model_weights}")
    print("#"*20)
    ...
    if args.is_pop909:
        ...
        for i in range(len(dataset)):
            print(dataset.data_files_chord[i].split("/")[-1][:-4])
            if args.test_id == dataset.data_files_chord[i].split("/")[-1][:-4]:
                test_id_idx = i
        if test_id_idx == -1:
    ...
    print("#" * 20)
    print(f"use_model:vmmixerm")
    print("#" * 20)
    model = VideoMusicMixerMamba(n_layers=args.n_layers, num_heads=args.num_heads,
                                 d_model=args.d_model, dropout=0.1, total_vf_dim=total_vf_dim).to(get_device())
    ...
    with torch.set_grad_enabled(False):
        if(args.beam > 0):
            ...
            if args.is_video:
                if args.is_datasetmidi:
                    rand_seq = dataset[test_id_idx]['x'].unsqueeze(0)

                elif isPrimer and len(custumPrimer) == 0:
            ...
            fs = FluidSynth(sound_font='soundfonts/default_sound_font.sf2')
...
if __name__ == "__main__":
    ...
    test_path = "dataset/vevo_meta/split/v1/VMM.txt"
